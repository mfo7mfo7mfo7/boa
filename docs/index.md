# Boa Documentation

Boa is a journey visualization platform. These guides are the field notes for tracing, observing, and guiding a release across the stars.

## Product

- [Release Notes 2.2](release-notes-2.2.md) — paper dialogs, hover notes, horizon labels, and public email links
- [Release Notes 2.1](release-notes-2.1.md) — automation client and integration examples
- [Release Notes 2.0](release-notes-2.0.md) — Boa Universe, Observation Notebook, Starlight, Storms, Marks, Engine Room
- [Release Notes 1.8](release-notes-1.8.md)
- [Release Notes 1.7](release-notes-1.7.md)
- [Release Notes 1.6](release-notes-1.6.md)
- [Release Notes 1.5](release-notes-1.5.md)
- [Release Notes 1.0](release-notes-1.0.md)

## Testing

- [Testing Strategy](testing.md) — how Boa is tested, including sandbox notes

## Design

- [Design System](design.md) — palette, typography, shapes, motion, and vocabulary

## Automation

- [Observation Client](automation-client.md) — write readings into existing journeys from Python, CLI, or n8n
- [Observation Client AI Prompt](observation-client-ai-prompt.md) — guardrails for AI-assisted integrations

## API

- [Bug Wave API](bug-wave.md) — known troubles and bug snapshots
- [Starlight API](starlight.md) — journey readiness
- [Email Ack Workflow](email-ack.md) — secret links and acknowledgements
- [Reminder Engine](reminders.md) — scheduled reminders and notifications

## Concepts

| Term | Meaning |
|------|---------|
| Journey | A release traced across time |
| Observation | "Where are we now?" |
| Starlight | Journey readiness |
| Bug Wave / Current Storms | Known troubles |
| Reminders | Email prompts before, during, and after milestones |
| Acknowledgement | Human confirmation of a milestone |
